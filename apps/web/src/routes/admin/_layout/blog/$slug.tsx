  ...
  if (view === "preview") {
    ...
    return (
      <div className="w-full max-w-4xl mx-auto">
        ...
        {tokensUsed > 0 && (
          <p className="text-xs text-muted-foreground mt-8">IA: {tokensUsed} tokens consumidos (Mistral)</p>
        )}
      </div>
    );
  }
    ...
    <div className="w-full max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8 sticky top-0 bg-background/90 backdrop-blur-sm z-10 py-3">
        <div className="flex items-center gap-2">
              ...
              <TooltipTrigger
                render={
                  <Button
                    ...
                    disabled={generatingExcerpt || (!contentEs && !contentHtml)}
                  />
                }
              >
          ...
          {tokensUsed > 0 && <span className="text-xs text-muted-foreground">({tokensUsed} tokens)</span>}
        </div>
        <div className="flex items-center gap-2">
          ...
          {post.status === "published" && (
            <Button variant="ghost" size="sm" onClick={() => unpublishPost({ postId: post._id })}>
              Despublicar
            </Button>
          )}
          <Button
      ...
      <div className="relative">
        ...
        />
        {capitalizing && <Loader2 className="absolute right-0 top-4 size-4 animate-spin text-muted-foreground" />}
      </div>

      <p className="text-sm text-muted-foreground mb-4">/{slugify(titleEs) || "..."}</p>

      {excerptEs && (
        <p className="text-lg text-muted-foreground italic mb-6 border-l-2 border-foreground/10 pl-4">{excerptEs}</p>
      )}
