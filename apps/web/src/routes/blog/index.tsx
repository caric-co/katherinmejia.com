...
export const Route = createFileRoute("/blog/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Artículos sobre maquillaje profesional, tendencias y consejos de belleza por Katherin Mejia",
      },
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post) => (
                <Link key={post._id} to={`/blog/${post.slug[locale]}` as string} className="group">
                  ...
                  <p className="text-muted-foreground line-clamp-2">{post.excerpt[locale]}</p>
                  {post.publishedAt && (
