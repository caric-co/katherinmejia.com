import { AnimatePresence, motion, type useAnimationControls } from "motion/react";
...
export function SmartSubmit({
        ...
        <AnimatePresence>
          <motion.p
            ...
            transition={{
              ...
              clipPath: { duration: 0.4, delay: 0.15 },
            }}
  ...
  if (emptyFieldLabels.length) {
    return (
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger render={<div className="w-full" />}>{submitButton}</TooltipTrigger>
          <TooltipContent className="block">
            <p className="mb-1">Completa los campos:</p>
            <ul className="list-disc pl-4">
              {emptyFieldLabels.map((f) => (
                <li key={f}>{f}</li>
              ))}
            </ul>
          </TooltipContent>
