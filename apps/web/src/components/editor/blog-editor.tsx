...
export const BlogEditor = forwardRef<BlogEditorRef, BlogEditorProps>(function BlogEditor(
  { initialContent, initialHtml, onChange, className },
  ref,
) {
    ...
    <EditorRoot>
      ...
      >
        ...
        <EditorCommand className="z-50 h-auto max-h-80 overflow-y-auto bg-card border border-border shadow-lg">
          <EditorCommandEmpty className="px-4 py-3 text-muted-foreground">Sin resultados</EditorCommandEmpty>
          <EditorCommandList>
            {slashItems.map((item) => (
              <EditorCommandItem
                key={item.title}
              ...
              >
                <div className="flex items-center justify-center size-9 bg-muted rounded-md shrink-0">{item.icon}</div>
                <div>
