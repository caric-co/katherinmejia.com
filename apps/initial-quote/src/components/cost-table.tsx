...
export function CostTable({ trm }: CostTableProps) {
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="text-right font-mono">
              <span className={fmt.isGreen ? "text-foreground" : ""}>
                {fmt.label}
              </span>
              {fmt.sub && (
  ...
  const serviceFooter = (
    <TableRow className="bg-muted font-semibold">
      <TableCell colSpan={4}>
        Total estimado mensual @ {formatNumber(selectedScale)} usuarios
      </TableCell>
    ...
    () => [
      ...
      ...userScales.map(
        (scale): ColumnDef<ServiceCost> => ({
          id: `scale-${scale}`,
          header: () => (
            <div className="text-right">{formatNumber(scale)}</div>
          ),
          cell: ({ row }) => {
            const raw = row.original.monthlyCost[scale] ?? 0
            const cop = toCOP(raw, row.original.currency, trm)
            return (
              <div className="text-right font-mono">{formatCOP(cop)}</div>
            )
          },
        })
      ),
      ...
      <TableRow className="bg-muted font-semibold">
        ...
        {userScales.map((scale) => {
          const total = billableServices.reduce(
            (sum, s) => sum + toCOP(s.monthlyCost[scale] ?? 0, s.currency, trm),
            0
          )
          return (
  ...
  return (
    <section className="mb-12">
      ...
      <p className="text-muted-foreground mb-6">
        ...
        originales en dólares se convierten a pesos colombianos con una TRM de{" "}
        {formatCOP(trm)}.
      </p>
      ...
      <div className="mt-8">
        ...
        <p className="text-xs text-muted-foreground mt-2">
          * Precios originales en dólares convertidos con TRM de{" "}
          ...
          $300 COP); no genera costo fijo mensual. Better Auth es código abierto
          (gratuito).
        </p>
      </div>
    </section>
