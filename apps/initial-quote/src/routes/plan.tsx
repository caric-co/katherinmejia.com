...
const phases = [
  ...
  {
    n: 3,
    title: "Página de Marca Personal",
    hours: "~8h",
    items: [
  ...
  {
    n: 4,
    title: "Schema Completo + Admin Base",
    hours: "~4h",
    items: [
  ...
  {
    n: 6,
    title: "Catálogo de Cursos (Público)",
    hours: "~4h",
    items: [
  ...
  {
    n: 7,
    title: "Reproductor de Video + Progreso",
    hours: "~6h",
    items: [
  ...
  {
    n: 9,
    title: "Contenido del Sitio + Blog",
    hours: "~2h",
    items: [
...
const publicRoutes = [
  ...
  ["/courses/$slug/lesson/$id", "Reproductor"],
  ["/courses/invite/$code", "Redimir invitación"],
...
const schemas: { name: string; fields: [string, string, string][] }[] = [
  {
    ...
    fields: [
      ...
      ["name", "string", ""],
      ["role", '"student" | "admin"', "Índice por rol"],
      ["avatar", "string?", "URL de imagen"],
      ["authProvider", '"email" | "google" | "apple"', ""],
      ["locale", '"es" | "en"', "Idioma preferido"],
      ["isBlocked", "boolean", "Admin puede bloquear"],
    ],
  },
  {
    name: "courses",
    fields: [
      ...
      ["slug", "string", "URL-friendly, índice"],
      ["thumbnailUrl", "string?", "UploadThing"],
      ["price", "number", "En COP"],
  ...
  },
  {
    name: "lessons",
    fields: [
      ["courseId", 'Id<"courses">', "Relación"],
      ["title", "{ es, en }", "Bilingüe"],
      ["videoId", "string", "Bunny Stream video ID"],
      ["duration", "number", "Segundos"],
      ["order", "number", "Dentro del curso"],
      ["isFree", "boolean", "Vista previa gratuita"],
    ],
  },
  {
    name: "progress",
    fields: [
      ["userId", "string", ""],
      ["lessonId", 'Id<"lessons">', ""],
      ["courseId", 'Id<"courses">', "Para consultas por curso"],
      ["watchedSeconds", "number", "Progreso por segundo"],
      ["completed", "boolean", "true cuando ≥90% visto"],
      ["lastWatchedAt", "number", "Para reanudar"],
    ],
  },
  {
    name: "purchases",
    fields: [
      ["userId", "string", ""],
      ["courseId", 'Id<"courses">', ""],
      ["amount", "number", "0 para invitaciones/manual"],
      ["provider", '"bold" | "manual" | "invitation"', ""],
      ["status", '"pending" | "completed" | "refunded"', ""],
      ["grantedBy", '"admin" | "invitation" | "payment"', "Unifica verificación"],
    ],
  },
  {
    name: "subscriptions",
  ...
  },
  {
    ...
    fields: [
      ["courseId", 'Id<"courses">', ""],
      ["code", "string", "Código único, índice"],
      ["maxUses", "number", ""],
      ["usedCount", "number", ""],
      ["expiresAt", "number?", "Opcional"],
    ],
  },
  {
    name: "siteContent",
    fields: [
      ["key", "string", 'Ej: "hero.title"'],
      ["value", "{ es, en }", "Bilingüe"],
  ...
  },
  {
    ...
    fields: [
      ...
      ["content", "{ es, en }", "Markdown o richtext"],
      ["excerpt", "{ es, en }", "Resumen para listado"],
...
const decisions = [
  [
    "🎬",
    "hls.js sobre iframe de Bunny",
    "Necesitamos eventos timeupdate del elemento <video> para seguimiento por segundo. El iframe requeriría postMessage, que es frágil.",
  ],
  [
    "⏱",
    "Debounce de 10 segundos",
    "Enviar progreso cada 10s equilibra precisión de reanudación con volumen de escrituras. A 100 estudiantes simultáneos: ~10 mutaciones/seg, dentro del plan gratuito de Convex.",
  ],
  [
    "🌐",
    "Datos bilingües como { es, en }",
    "Objetos directos en Convex en lugar de tabla de traducciones. Consultas simples, sin joins. Aceptable porque solo hay 2 idiomas.",
  ],
  [
    "📝",
    "i18n híbrido",
    "Strings estáticos de UI vía archivos JSON de i18next (rápido, sin consultas DB). Contenido dinámico (hero, testimonios) vía tabla siteContent (editable por admin).",
  ],
  [
    "🎟️",
    "Invitaciones como registros de compra",
    "Al redimir, se crea un purchases con grantedBy: 'invitation' y amount: 0. La verificación de acceso solo consulta purchases.",
  ],
  [
    "🖼",
    "UploadThing para imágenes, Bunny para video",
    "UploadThing para miniaturas y portadas de blog (free tier 2GB). Bunny Stream exclusivamente para video (HLS + DRM).",
  ],
  [
    "🔒",
    "Auth guard en beforeLoad",
    "TanStack Router ejecuta beforeLoad antes de renderizar. Se consulta el usuario, se verifica el rol y se lanza redirect() si no tiene permisos. Sin destello de contenido no autorizado.",
  ],
];
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        <p className="text-muted-foreground mb-10 max-w-3xl">
          ...
          ). Este documento detalla las 10 fases de implementación del MVP, la arquitectura, el modelo de datos y las
            ...
            <ul className="space-y-1">
              {publicRoutes.map(([path, desc]) => (
                <li key={path} className="text-sm">
                  <code className="text-foreground bg-muted px-1.5 py-0.5 text-xs">{path}</code>{" "}
